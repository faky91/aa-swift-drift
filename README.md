...
pip install git+https://github.com/faky91/aa-swift-drift.git@v0.16.1
...
```
aa-swift-drift @ git+https://github.com/faky91/aa-swift-drift.git@v0.16.1
```
...
autopilot waypoints and can read the pilot's current location for the
"use my current location" button. This uses the ESI scopes
`esi-ui.write_waypoint.v1` and `esi-location.read_location.v1`, which
must be added to the EVE application
